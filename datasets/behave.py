...
class BehaveData():
    def __init__(self, cfg):
        ...
        self.human_motion_data_path = os.path.join(data_root, 'behave_t2m/new_joints_local/')
        self.object_motion_data_path = os.path.join(data_root, 'behave_t2m/object_joints_local/')
        self.text_path = os.path.join(data_root, 'behave_t2m/texts/')
    ...
    def get_data_by_name(self, name, start_frame, augment=False):
        ...
        txt_path = os.path.join(self.text_path, name + '.txt')
        with open(txt_path, 'r') as f:
            text_list = [line.strip() for line in f.readlines()]
        text = random.choice(text_list)
        text = text.split('#')[0]

        human_motion = np.load(os.path.join(self.human_motion_data_path, name + '.npy'))[0]
