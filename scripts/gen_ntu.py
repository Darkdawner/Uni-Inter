"""Generate predictions for NTU evaluation."""
import sys
import os
...
class NTUTestDataset(Dataset):
    def __init__(self, cfg):
        self.dbs = json.load(open(os.path.join(
            os.path.dirname(__file__), os.pardir, 'data_splits/ntu_test.json'
        )))
        self.max_length = cfg.max_frames
        self.NTUData = NTUData(cfg, mode='test')
    ...
    def __getitem__(self, idx):
        ...
        human_voxel_index, cond_voxel_grid, object_motion, valid_mask, text = \
            self.NTUData.get_data_by_name(name, start_frame, augment=False)
...
def main():
    test_dataset = NTUTestDataset(cfg)
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            # Undo mirror flip and axis swap
