"""Generate predictions for MANIP (OMOMO) evaluation."""
import sys
import os
...
from datasets import ManipData
...
class ManipTestDataset(Dataset):
    def __init__(self, cfg):
        self.dbs = json.load(open(os.path.join(
            os.path.dirname(__file__), os.pardir, 'data_splits/manip_test.json'
        )))
        self.max_length = cfg.max_frames
        self.ManipData = ManipData(cfg, mode='test')
    ...
    def __getitem__(self, idx):
        ...
        human_voxel_index, cond_voxel_grid, object_motion, valid_mask, text = \
            self.ManipData.get_data_by_name(name, start_frame, augment=False)
...
def main():
    test_dataset = ManipTestDataset(cfg)
    ...
    mirror_flip = [0, 2, 1, 3, 5, 4, 6, 8, 7, 9, 11, 10, 12, 14, 13, 15, 17, 16, 19, 18, 21, 20]
    output_dir = './eval_manip/pred/'
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            # Undo axis swap and mirror flip
            motion_pred = motion_pred[:, mirror_flip, :]
            motion_pred = motion_pred[:, :, [0, 2, 1]]
